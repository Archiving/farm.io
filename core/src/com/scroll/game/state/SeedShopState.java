	...
	public void render(SpriteBatch sb) {
		...
		for(int row = 0; row < crops.length; row++) {
			for(int col = 0; col < crops[0].length; col++) {
				...
				smallFont.draw(sb, "$" + crops[row][col].cost, 180 + 67 * col - ((crops[row][col].cost >= 10) ? 5 : 0)- ((crops[row][col].cost >= 100) ? 5 : 0), 265 - 65 * row);
				smallFont.draw(sb, crops[currentRow][currentCol].toString().substring(0,1) + crops[currentRow][currentCol].toString().substring(1).toLowerCase() + " Seed", 335, 360);
				if(Arrays.asList(selectedRegion.affectedCrops).contains(crops[currentRow][currentCol])) {
					smallFont.draw(sb, "Growth Time: ~" + (int)(crops[currentRow][currentCol].requiredTime/selectedRegion.timeRatio) + " hours", 290, 345);
					smallFont.draw(sb, "Special Crop!", 325, 330);
					medFont.setColor(Color.GREEN);
				}
				else { 
					smallFont.draw(sb, "Growth Time: ~" + (int)(crops[currentRow][currentCol].requiredTime) + " hours", 290, 345);
